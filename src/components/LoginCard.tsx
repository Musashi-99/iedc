...
export default function LoginCard() {
  ...
  return (
    <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl shadow-slate-900/10 ring-1 ring-black/5">
      ...
      <div className="mt-5 space-y-3">
        <div className="flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2.5 focus-within:border-brand-blue">
        ...
        <div className="flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2.5 focus-within:border-brand-blue">
          ...
          <input
            ...
            className="w-full text-sm text-slate-700 outline-none placeholder:text-slate-400"
          />
          <button onClick={() => setShowPassword((s) => !s)} className="text-slate-400 hover:text-slate-600">
            {showPassword ? <EyeOff size={16} /> : <Eye size={16} />}
          </button>
        </div>
      </div>

      <button className="mt-5 w-full rounded-xl bg-brand-blue py-3 text-sm font-semibold text-white shadow-md shadow-brand-blue/30 hover:bg-navy-800">
        Login
      </button>

      <button className="mt-3 flex w-full items-center justify-center gap-2 text-sm font-medium text-brand-blue hover:underline">
        <LogOut size={14} />
        Logout
      </button>
    </div>
